...
class Test(TestCase):
	def test_tag_parts_of_speech(self):
		self.assertEqual(SENTENCE_ONE_RES, post.tag_parts_of_speech(SENTENCE_ONE))

	def test_run(self):
		self.assertEqual([SENTENCE_ONE_RES, SENTENCE_TWO_RES], post.run([SENTENCE_ONE, SENTENCE_TWO]))
